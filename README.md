...
从沪深涨幅榜筛选候选，按 **技术40% + 低位25% + 情绪20% + 增长15%** 四维综合打分排序，输出推荐榜与每只股票的推荐理由。
...
- 🎯 **多维度选股推荐**：按「技术40% + 低位25% + 情绪20% + 增长15%」四维综合打分排序，输出今日推荐榜
...
从沪深涨幅榜筛选候选，逐只抓取行情/新闻/基本面，按 **四维综合打分** 排序输出推荐榜：

| 维度 | 权重 | 计算依据 |
|---|---|---|
| 技术面 | 40% | 均线、MACD、RSI、KDJ、量能、布林 6 因子信号分 |
| 低位买入信号 | 25% | 距60日高点回撤、区间位置、布林下轨/RSI超卖、低位放量 |
| 情绪面 | 20% | 新闻情绪分（利好/利空） |
| 未来增长 | 15% | 净利/营收同比、增长趋势、ROE、毛利率、PEG |

可选参数：`--min-price` / `--max-price` 过滤价格区间；`--fs` 过滤板块（默认沪深A股）。
...
5. **选股推荐**：`screen.mjs` 四维综合打分排序，输出今日推荐榜
