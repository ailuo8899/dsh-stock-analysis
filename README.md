...
单股报告新增「位置研判（双向）」区块：位置分区徽章 + 买卖倾向 + 买/卖双分数 + 低位买入/高位卖出信号明细。

![位置研判](assets/position-panel.png)
...
- 📍 **双向位置研判**：同时输出低位买入信号与高位卖出信号（买X/卖X 双分 + 位置分区 + 高低位买卖倾向）
- 📉 **支撑/压力位**：布林带、斐波那契、均线自动标注（严格支撑<现价<压力，杜绝倒挂）
- 💰 **持仓盈亏计算**：输入「股数 + 成本价」即算浮盈/浮亏金额与比例，并给出参考止损/止盈位；**高位卖出信号强时联动提示止盈/减仓**
- 📊 **可视化交付**：内嵌 SVG K 线图 + 完整 HTML 报告（含交互式盈亏计算器、位置研判 panel）
- 📈 **未来增长评分**：净利/营收同比 + 增长趋势 + ROE + 毛利率 + PEG 基本面多维打分
- ⚠️ **追高防护**：当日涨幅 ≥5% 自动追加追高风险提示；涨停/接近涨停标记「追高风险极大」
- 🎯 **多维度选股推荐**：按「技术30% + 低位20% + 情绪15% + 增长15% + 动量20%」五维综合打分排序，输出低位买入候选与高位卖出提醒
...
│   ├── analyze.mjs     # 技术指标 + 多因子信号 + 情绪 + 未来增长 + 双向位置研判 + 盈亏
│   ├── render.mjs      # SVG K线图 + HTML 报告（含位置研判/增长区块）+ markdown 摘要
